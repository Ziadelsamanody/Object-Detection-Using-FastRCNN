...
def match_rois_to_gt(rois, gt_boxes, gt_labels, pos_iou_thresh=0.5, neg_iou_thresh=0.1):
    """
    Match ROIs to ground truth boxes based on IoU
    
    Args:
        rois: Tensor [N, 5] - proposals (can be on CUDA)
    ...
    Returns:
        ...
        matched_boxes: Tensor [N, 4] - matched GT boxes
    """
    ...
    for i, roi in enumerate(rois_cpu):
        roi_box = roi[1:].numpy()  # Remove batch index
        max_iou = 0
        max_idx = -1
